...
public final class Auton extends SubsystemBase {
    ...
    private static final Map<String, Command> commands = Map.ofEntries(
        ...
        //     .andThen(sleep(1))
        ...
        ), entry("04 - pos2 (almost) TWO algae",
            new ParallelCommandGroup(
                drive.followPathCommand(/*"Position2"*/"BargeToFrontReefTest", true),
                algae.upCommand()
                .andThen(sleep(.75))
                .andThen(elevator.stepToCommand(Position.L4))
            )
            .andThen(sleep(.3)) // give time for robot to stabilize after bringing elevator up (was .5)
            .andThen(coral.toggleCommand())
            .andThen(sleep(.2)) // was .3
            .andThen(new ParallelCommandGroup(
                drive.followPathCommand("FrontCoralToAlgae", false),
                elevator.stepToCommand(Position.LOAD)
            ))
            ...
            .andThen(new ParallelCommandGroup(
                ...
                sleep(.25)
                .andThen(elevator.stepToCommand(Position.BARGE))
            )) // end of "pos 2 1 algae" stuff
            .andThen(algae.toggleDriveCommand())
            .andThen(sleep(.2)) // was .4
            .andThen(new ParallelCommandGroup(
            ...
            .andThen(sleep(.2))
            .andThen(new ParallelCommandGroup(
                drive.followPathCommand("SpinFromRightReef", false),
                sleep(.3)
